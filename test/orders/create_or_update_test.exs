defmodule Exlivery.Ordes.CreateOrUpdateTest do
  use ExUnit.Case

  import Exlivery.Factory

  alias Exlivery.Ordes.CreateOrUpdate
  ...
  describe "call/1" do
    ...
    test "when there is no user with given cpf, returns an error", %{item1: item1, item2: item2} do
      params = %{user_cpf: "00987654321", items: [item1, item2]}
     ...
     test "when there ara invalid itens, returns an error", %{
      user_cpf: cpf,
      item1: item1,
      item2: item2
    } do
    ...
    test "when there ara no itens, returns an error", %{user_cpf: cpf} do
